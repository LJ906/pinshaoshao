    ...
    <div v-if="userInfo.id">
         ...
         <section>
          <div class="shopCart_list_con" v-for="(goods, index) in cartgoods" :key="index">
            <div class="list_con_left">
              ...
              @click.stop="handleCheckedSingle(goods)"
              ></a>
            </div>
            <div class="list_con_right">
              ...
              <div class="shop_con">
                ...
                <p class="shop_price">{{goods.price | priceFormat(goods.price)}}</p>
                <div class="shop_deal">
                  <div class="shop_deal_left">
                    <span @click.stop="changeCartCount({goods: goods, flag: false})">-</span>
                    <input type="tel" value="1" v-model="goods.buy_count" disabled>
                    <span @click.stop="changeCartCount({goods: goods, flag: true})" >+</span>
                  </div>
                  <div class="shop_deal_right" @click="deleteCartGoods(goods)">
                    <span></span>
                    <span></span>
                  </div>
                </div>
      ...
      <div id="tab_bar">
        <div class="tab-bar-left">
          ...
          @click.stop="handleCheckAll(isCheckedAll)" ></a>
          <span style="font-size: 16px;">全选</span>
          <div class="select-all">合计：
            <span class="total-price">{{totalPrice | priceFormat}}</span>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    totalPrice () {
      ...
      this.cartgoods.forEach(goods => {
        if (goods.checked) {
          sumPrice +=goods.price * goods.buy_count
        }
      })
  ...
  methods: {
    ...mapActions(['changeCartCount', 'handleCheckedSingle', 'handleCheckAll', 'deleteCartGoods']),     
  }, 
  components: {
    SelectLogin
  },
  filters: {
    priceFormat (price) {
      var source = price.toString().split(".");
      source[0] = source[0].replace(/(\d)(?=(\d{3})+$)/gi,"$1,");
      // 小数部分
  
      if(source[1]) {
        var rightLen = source[1].length;
        var zero = ''
        if (rightLen && rightLen <=2 ) {           
           zero = rightLen == 0 ? '00' : rightLen == 1 ? '0': ''
        } 
        source[1] += zero;
      }else { 
        zero = '00' 
